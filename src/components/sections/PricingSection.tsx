...
const pricingCtaShellClass = SHOW_PRICING_CTA ? "mt-8" : "mt-8 hidden h-0 overflow-hidden";

function CheckIcon() {
  return (
    <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-brand-accent">
      <svg className="h-2.5 w-2.5 text-brand-ink" viewBox="0 0 12 12" fill="none" aria-hidden>
        <path
          d="M2 6l2.5 2.5L10 3"
          stroke="currentColor"
          strokeWidth="2"
  ...
  return (
    <ScrollFade className="w-full">
      ...
      >
        ...
        <h2
          id="pricing-heading"
          className="mt-3 text-center text-3xl font-bold tracking-tight text-brand-ink md:text-4xl"
        >
          {pricing.title}
        </h2>
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-6 flex flex-1 flex-col gap-4">
                    {tier.headline?.trim() ? (
                      <p className="text-2xl font-bold leading-tight text-brand-paper md:text-3xl">{tier.headline}</p>
                    ) : null}
                    {tier.body?.trim() ? (
                      <p className="text-sm leading-relaxed text-brand-paper/90 md:text-base">{tier.body}</p>
                    ) : null}
                    ...
                    {tier.howItWorks?.trim() ? (
                      <div>
                        ...
                        <p className="mt-2 text-sm leading-relaxed text-brand-paper/85">{tier.howItWorks}</p>
                      </div>
                    ) : null}
                  </div>
                  ...
                  {tier.footnote?.trim() ? (
                    <p className="mt-4 text-center text-xs leading-relaxed text-brand-paper/80">{tier.footnote}</p>
                  ) : null}
                </article>
              );
            }
              ...
              >
                ...
                <div className="mt-5">
                  <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                    {tier.compareAtPrice ? (
                      <span className="text-lg text-brand-muted-light line-through">{tier.compareAtPrice}</span>
                    ) : null}
                    <span className="text-3xl font-bold text-brand-ink md:text-4xl">{tier.price}</span>
                    <span className="text-sm font-medium text-brand-muted">{tier.pricePeriod}</span>
                  </div>
                </div>
                ...
                {tier.footnote?.trim() ? (
                  <p className="mt-3 text-center text-[11px] text-brand-muted-light">{tier.footnote}</p>
                ) : null}
              </article>
            );
          })}
        ...
        {pricing.footerCta ? (
          <div className="mx-auto mt-12 max-w-2xl border-t border-brand-border pt-10 text-center md:mt-14 md:pt-12">
